...
SESSION_TTL_SECONDS    = 3600       # 1 hour  (general documents)
SESSION_TB_TTL_SECONDS = 8 * 3600  # 8 hours (template-builder — must outlive WS timeout)
...
def _builder_forcesave(session_id: str, key: str) -> dict[str, Any]:
    token = jwt.encode({"c": "forcesave", "key": key}, _onlyoffice_jwt_secret(), algorithm="HS256")
    payload = {"token": token}
...
def template_builder_session(session_id: str):
    ...
    ws_url = f"{scheme}://{host}{base}/api/template-builder/{session_id}/ws"
...
@word_constructor.get("/template-builder/google-doc/")
def template_builder_google_doc():
    return render_template(
        "word_constructor/template_builder_google_doc.html",
        google_docs_url="https://docs.google.com/document/create?usp=docs_web",
...
def api_template_builder_onlyoffice_config(session_id: str):
    ...
    # Inject sidebar plugin — session_id lets it fetch params from the right session
    _pub_scheme = "https" if request.headers.get("X-Forwarded-Proto", request.scheme) == "https" else "http"
    _plugin_url = (
        f"{_pub_scheme}://{request.host}"
        f"/services/word-constructor/onlyoffice-plugin/config.json?session_id={session_id}"
    )
    _plugin_guid = "asc.{b3c7e1a2-4d5f-4890-bcde-f12345678901}"
    payload["editorConfig"]["plugins"] = {
        "pluginsData": [_plugin_url],
        "autostart": [_plugin_guid],   # open panel automatically when document loads
    }
...
def api_1c_documents_bridge():
    ...
    return jsonify({
        ...
        "websocket_url": f"{scheme}://{host}{base}/api/template-builder/{session_id}/ws",
        "expires_at":    meta["expires_at_iso"],
    }), 201
